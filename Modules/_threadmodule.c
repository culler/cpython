...
rlock_is_owned_by(rlockobject *self, PyThread_ident_t tid)
{
    PyThread_ident_t owner_tid = (PyThread_ident_t)
      _Py_atomic_load_uint64_relaxed(&self->rlock_owner);
...
rlock_acquire(rlockobject *self, PyObject *args, PyObject *kwds)
{
    ...
    if (r == PY_LOCK_ACQUIRED) {
        ...
        _Py_atomic_store_uint64_relaxed(&self->rlock_owner, tid);
        self->rlock_count = 1;
    }
...
rlock_release(rlockobject *self, PyObject *Py_UNUSED(ignored))
{
    ...
    if (--self->rlock_count == 0) {
      _Py_atomic_store_uint64_relaxed(&self->rlock_owner, 0);
        PyThread_release_lock(self->rlock_lock);
    }
    Py_RETURN_NONE;
}
...
rlock_acquire_restore(rlockobject *self, PyObject *args)
{
    ...
    assert(self->rlock_count == 0);
    _Py_atomic_store_uint64_relaxed(&self->rlock_owner, owner);
    self->rlock_count = count;
    Py_RETURN_NONE;
}
...
thread__make_thread_handle(PyObject *module, PyObject *identobj)
{
    ...
    PyThread_ident_t ident = (PyThread_ident_t)
        PyLong_AsUnsignedLongLong(identobj);
    if (PyErr_Occurred()) {
        return NULL;
    }
